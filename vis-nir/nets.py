...
def Encoder_hot(images, image_size, cat_num, hot_size, reuse=False, is_training=True, name='Encoder_hot'):
    # image_size = 32
    init_size = int(image_size/32)
    with tf.variable_scope(name, reuse=reuse):
        with slim.arg_scope([slim.conv2d], padding='SAME', activation_fn=None, stride=2,
            ...
            with slim.arg_scope([slim.batch_norm], decay=0.95, center=True, scale=True,
                ...
                # [batch_size, image_size/2, image_size/2, 8]
                net = slim.batch_norm(net, scope='bn1')
                net = slim.conv2d(net, 32, [3, 3], scope='conv2')
                # [batch_size, image_size/4, image_size/4, 32]
                net = slim.batch_norm(net, scope='bn2')
                net = slim.conv2d(net, 64, [3, 3], scope='conv3')
                # [batch_size, image_size/8, image_size/8, 64]
                mid = slim.batch_norm(net, scope='bn3')
                net = slim.conv2d(mid, 128, [3, 3], scope='conv4')
                # [batch_size, image_size/16, image_size/16, 128]
                net = slim.batch_norm(net, scope='bn4')
                net = slim.conv2d(net, 512, [3, 3], scope='conv5')
                # [batch_size, image_size/32, image_size/32, 512]
                net = slim.batch_norm(net, scope='bn5')
                net = slim.conv2d(net, hot_size, [init_size, init_size], padding='VALID', scope='conv6')
                # [batch_size, 1, 1, hot_size]
                net = slim.batch_norm(net, scope='bn6')
                hot_code = slim.flatten(net)  # [batch_size, hot_size]
                logits = slim.fully_connected(hot_code, cat_num, activation_fn=None, scope='logits')
                # [batch_size, cat_num]
                prob = tf.nn.softmax(logits, name='hot_code')
                return mid, hot_code, logits, prob
    ...
    with tf.variable_scope(name, reuse=reuse):
        with slim.arg_scope([slim.conv2d], padding='SAME', activation_fn=None, stride=2,
            ...
            with slim.arg_scope([slim.batch_norm], decay=0.95, center=True, scale=True,
                                activation_fn=tf.nn.relu, is_training=is_training):
                net = slim.conv2d(mid, 128, [3, 3], scope='conv4')
                # [batch_size, image_size/16, image_size/16, 32]
                net = slim.batch_norm(net, scope='bn4')
                net = slim.conv2d(net, 256, [3, 3], scope='conv5')
                # [batch_size, image_size/32, image_size/32, 16]
                net = slim.batch_norm(net, scope='bn5')
                net = slim.conv2d(net, 128, [1, 1], stride=1, scope='conv6')
                # [batch_size, image_size/32, image_size/32, 128]
                net = slim.batch_norm(net, scope='bn6')
                calm_code = slim.flatten(net)
                calm_code = slim.fully_connected(calm_code, calm_size, activation_fn=None,
                                                 scope='calm_code')
                # [batch_size, calm_size]
                # calm_code = tf.nn.sigmoid(calm_code)
                return calm_code
    ...
    with tf.variable_scope(name, reuse=reuse):
        with slim.arg_scope([slim.conv2d_transpose], padding='SAME', activation_fn=None, stride=2,
            ...
            with slim.arg_scope([slim.batch_norm], decay=0.95, center=True, scale=True,
                ...
                net = slim.batch_norm(net, scope='bn1')
                net = slim.conv2d_transpose(net, 512, [3, 3], scope='conv_t2')
                # [batch_size, 2*init_size, 2*init_size, 512]
                net = slim.batch_norm(net, scope='bn2')
                net = slim.conv2d_transpose(net, 256, [3, 3], scope='conv_t3')
                # [batch_size, 4*init_szie, 4*init_size, 256]
                net = slim.batch_norm(net, scope='bn3')
                net = slim.conv2d_transpose(net, 128, [3, 3], scope='conv_t4')
                # [batch_size, 8*init_szie, 8*init_size, 128]
                net = slim.batch_norm(net, scope='bn4')
                net = slim.conv2d_transpose(net, 64, [3, 3], scope='conv_t5')
                # [batch_size, 16*init_szie, 16*init_size, 64]
                net = slim.batch_norm(net, scope='bn5')
                net = slim.conv2d_transpose(net, 32, [3, 3], scope='conv_t6')
                # [batch_size, 32*init_szie, 32*init_size, 32]
                net = slim.batch_norm(net, scope='bn6')
                net = slim.conv2d_transpose(net, 3, [1, 1], stride=1, activation_fn=tf.tanh,
                                            scope='conv_t7')
